...
export default class Product {
    ...
    static #columns = ['id', 'nome', 'codigo_barra', 'unidade', 'preco_compra', 'preco_venda', null, null];
    ...
    // Pesquisa paginada para o DataTable
    static async find(data = {}) {
        ...
        // Total sem filtro (apenas não excluídos)
    ...
    // Exclusão lógica (marca excluido = true)
    static async delete(id) {
        if (!id) return { status: false, msg: 'ID é obrigatório' };

        try {
            await connection(Product.table)
                .where({ id })
